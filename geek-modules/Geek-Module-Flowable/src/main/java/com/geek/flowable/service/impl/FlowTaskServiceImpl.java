package com.geek.flowable.service.impl;
...
import com.geek.flowable.common.constant.ProcessConstants;
import com.geek.flowable.common.enums.FlowComment;
import com.geek.flowable.domain.dto.FlowCommentDto;
import com.geek.flowable.domain.dto.FlowNextDto;
import com.geek.flowable.domain.dto.FlowTaskDto;
import com.geek.flowable.domain.dto.FlowViewerDto;
import com.geek.flowable.domain.vo.FlowQueryVo;
import com.geek.flowable.domain.vo.FlowTaskVo;
import com.geek.flowable.factory.FlowServiceFactory;
import com.geek.flowable.flow.CustomProcessDiagramGenerator;
import com.geek.flowable.flow.FindNextNodeUtil;
import com.geek.flowable.flow.FlowableUtils;
...
public class FlowTaskServiceImpl extends FlowServiceFactory implements IFlowTaskService {
    ...
    public AjaxResult flowRecord(String procInsId, String deployId) {
        ...
        if (StringUtils.isNotBlank(deployId)) {
            ...
            map.put("formData", com.geek.common.utils.JSON.parseObject(sysForm.getFormSchema()));
        }
    ...
    public AjaxResult flowFormData(String deployId) {
        ...
        if (StringUtils.isNotBlank(deployId)) {
            ...
            return AjaxResult.success(com.geek.common.utils.JSON.parseObject(sysForm.getFormSchema()));
        } else {
    ...
    public AjaxResult flowTaskForm(String taskId) throws Exception {
        ...
        ObjectNode oldVariables = (ObjectNode)JSON.parseObject(JSON.toJSONString(parameters.get("formJson")));
        ArrayNode oldFields = (ArrayNode)JSON.parseObject(JSON.toJSONString(oldVariables.get("widgetList")));
        // 设置已填写的表单为禁用状态
        for (JsonNode oldField : oldFields) {
            ObjectNode options = (ObjectNode) oldField.get("options");
        ...
        if (StringUtils.isNotBlank(task.getFormKey())) {
            ...
            JsonNode data = JSON.parseObject(sysForm.getFormSchema());
            ArrayNode newFields = (ArrayNode)JSON.parseObject(JSON.toJSONString(data.get("widgetList")));
            // 表单回显时 加入子表单信息到流程变量中
            for (JsonNode newField : newFields) {
                String key = newField.get("id").asText();
                // 处理图片上传组件回显问题
                if ("picture-upload".equals(newField.get("type").asText())) {
        ...
        oldVariables.set("widgetList", oldFields);
